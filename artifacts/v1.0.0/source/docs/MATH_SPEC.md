...
The v1.0.0 artifact uses both the fixed-cutoff CRT model and the diagonal
dyadic model. The fixed-cutoff model provides complete-CRT inputs, while the
main Angular Erdos-Kac theorem concerns the diagonal statistic on dyadic
windows such as `N < n <= 2N`.
